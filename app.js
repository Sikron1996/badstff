...
const CONTRACT_ADDRESS = "0x458Ee562DdD3C04a3492D769146B97122Cc7BccE";
...
async function loadSupply(){
  try{
    ...
    text("mintedBottom", supply.toLocaleString());

    const percent = Math.min(100, (supply / MAX_SUPPLY) * 100);
    text("progressText", percent.toFixed(1).replace(".0","") + "% MINTED");
...
async function updatePrice(){
  try{
    ...
    text("priceText", total === 0n ? "FREE / 0.0001 ETH" : ethers.formatEther(total) + " ETH");
  }catch(e){
...
if(randomBtn){
  randomBtn.onclick = () => {
    ...
    img.style.transform = "scale(1.03) rotate(" + ((Math.random() * 4) - 2) + "deg)";
